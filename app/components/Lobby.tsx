...
import { ArrowLeft, Play, Check, Clock3, Copy, Share2, QrCode, X, MessageSquare, Send } from 'lucide-react'
...
export default function Lobby({
  ...
  if (!detail || detail.id !== selectedId) return (
    <div className="mx-auto w-full max-w-xl p-4 sm:p-6 text-center" style={{ color: '#8B8FAD' }}>
      Loading lobby…
    </div>
  )
  ...
  return (
    <div className="mx-auto w-full max-w-2xl p-4 sm:p-6">
      {/* Back */}
      <button onClick={onBack} className="mb-5 flex items-center gap-2 text-sm transition-colors" style={{ color: '#8B8FAD' }}
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-[1fr_260px]">
        {/* Main panel */}
        <div className="rounded-2xl p-4 sm:p-5" style={{ background: '#1A1D35', border: '1px solid #2F3355' }}>
          {inProgress && isHost ? (
            <>
              <h2 className="font-semibold" style={{ color: '#F0F2FF' }}>Live leaderboard</h2>
              ...
              {leaderboard.length > 0 ? (
                <div className="mt-4 flex flex-col gap-1.5">
                  {leaderboard.map((l: any, i: number) => (
                    <div key={l.playerId} className="flex items-center gap-3 rounded-xl px-4 py-3 card-3d"
                      style={{ background: i === 0 ? 'rgba(233,178,19,0.1)' : '#252847', border: i === 0 ? '1px solid rgba(233,178,19,0.25)' : '1px solid transparent' }}>
                      <span className="flex size-8 items-center justify-center rounded-full text-xs font-bold"
                        style={{ background: i === 0 ? '#E9B213' : '#2F3355', color: i === 0 ? '#0D0F1F' : '#8B8FAD' }}>{i + 1}</span>
                      <span className="flex-1 truncate text-sm">{l.player?.displayName ?? formatAddress(l.player?.nimiqAddress ?? l.playerId)}</span>
                      <span className="font-mono text-sm" style={{ color: '#E9B213' }}>{l.correctCount} ✓</span>
                    </div>
                  ))}
                </div>
              ) : (
              ...
              {pot > 0 && <p className="mt-4 text-sm" style={{ color: '#8B8FAD' }}>Prize pot: <span className="font-mono font-bold" style={{ color: '#E9B213' }}>{pot} NIM</span></p>}
            </>
          ) : (
            <>
              <h2 className="font-semibold" style={{ color: '#F0F2FF' }}>
                Players <span className="font-normal text-sm" style={{ color: '#8B8FAD' }}>({confirmed.length} paid · {detail.entries.length} total)</span>
              </h2>
                ...
                {detail.entries.map((e) => (
                  <div key={e.id} className="flex items-center justify-between rounded-xl px-4 py-3"
                    ...
                    {e.stakeStatus === 'CONFIRMED' ? (
                      <span className="flex items-center gap-1 text-xs font-semibold" style={{ color: '#21BCA5' }}><Check className="size-3.5" /> Paid</span>
                    ) : e.stakeStatus === 'PENDING' ? (
                      <span className="flex items-center gap-1 text-xs" style={{ color: '#E9B213' }}><Clock3 className="size-3.5" /> Pending</span>
                    ) : (
              ...
              {isHost ? (
                <div className="mt-5 flex flex-col gap-2">
                  ...
                  {detail.status === 'OPEN' && confirmed.length === 0 && onDeleteRound && (
                    <Button variant="outline" onClick={onDeleteRound} disabled={busy}
                      className="h-10" style={{ borderColor: 'rgba(239,68,68,0.3)', color: '#EF4444' }}>
                      Cancel round
                    </Button>
                  )}
                </div>
        ...
        <div className="flex flex-col gap-3">
          {/* Share sheet */}
          <div className="rounded-2xl p-4" style={{ background: '#1A1D35', border: '1px solid #2F3355' }}>
            ...
            <div className="flex gap-2 mb-3">
              {shareLinks.map(link => (
                <a
                  ...
                  title={link.label}
                  className="flex flex-1 items-center justify-center rounded-xl py-2.5 text-base transition-all"
                  style={{ background: '#252847', border: '1px solid #2F3355' }}
            ...
            >
              <span className="min-w-0 truncate text-left">
                ...
                <span className="font-mono" style={{ color: '#8B8FAD' }}>/join/{detail.id.slice(0, 10)}…</span>
              </span>
              {copied
          ...
          <button
            ...
            style={{ background: 'rgba(233,178,19,0.08)', border: '1px solid rgba(233,178,19,0.25)', color: '#E9B213' }}
            onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background = 'rgba(233,178,19,0.15)' }}
            onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background = 'rgba(233,178,19,0.08)' }}
          >
      ...
      {showQR && (
        <div
          ...
          style={{ background: 'rgba(13,15,31,0.9)', backdropFilter: 'blur(8px)' }}
          onClick={() => setShowQR(false)}
        >
          <div
            className="relative w-full max-w-sm rounded-3xl p-8 text-center"
            style={{ background: '#1A1D35', border: '1px solid rgba(233,178,19,0.3)' }}
          ...
          >
            ...
            <QrCode className="mx-auto mb-3 size-8" style={{ color: '#E9B213' }} />
            <h3 className="text-lg font-bold">Scan to join</h3>
            <p className="mt-1 text-sm" style={{ color: '#8B8FAD' }}>Opens in Nimiq Pay automatically</p>

            <div className="mx-auto mt-6 w-fit rounded-2xl p-4" style={{ background: '#FFFFFF' }}>
              <QRCodeSVG
                value={joinUrl}
                size={200}
                level="M"
            ...
            <p className="mt-5 text-xs font-semibold" style={{ color: '#E9B213' }}>{detail.title}</p>
            <p className="mt-1 text-xs" style={{ color: '#8B8FAD' }}>Stake: {Number(detail.stakeAmount)} NIM · {detail.questionCount} questions</p>
          </div>
        </div>
      )}
    </div>
